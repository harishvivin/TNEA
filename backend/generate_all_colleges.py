...
prefixes = ["Sri", "Global", "Excel", "Pioneer", "Royal", "National", "Mother Teresa", "Maha", "Kalaignar", "Amrita", "Bharath", "Kamaraj", "Dr. Kalam", "Anna", "CVS", "Jayam", "Muthayammal", "SNS", "RVS", "Karpagam", "Hindusthan", "Vellalar"]
suffixes = ["Engineering College", "Institute of Technology", "College of Engineering", "Institute of Science and Technology"]
types = ["Autonomous", "Non-Autonomous", "Government", "Government-Aided"]
weights = [0.25, 0.65, 0.05, 0.05]

existing_sql = """INSERT INTO college (name, district, type, previous_year_cutoff) VALUES ('J K K Munirajah College of Technology', 'Erode', 'Non-Autonomous', 146.5);
...
with open("d:/TNEA/backend/src/main/resources/data.sql", "w") as f:
    f.write(existing_sql + "\n")
    
    # Generate 10-15 colleges per district to reach ~500 total
    for district in districts:
        num_colleges = random.randint(10, 15)
        for _ in range(num_colleges):
            prefix = random.choice(prefixes)
            suffix = random.choice(suffixes)
            name = f"{prefix} {random.choice(['City ', 'Regional ', ''])} {suffix} of {district}".replace('  ', ' ')
            col_type = random.choices(types, weights=weights)[0]
            cutoff = round(random.uniform(90.0, 199.5) * 2) / 2 # increments of 0.5
            f.write(f"INSERT INTO college (name, district, type, previous_year_cutoff) VALUES ('{name}', '{district}', '{col_type}', {cutoff});\n")

print("Generated ~500 colleges in data.sql")
